...
- **Потоковое шифрование/дешифрование:**
  - Автоматическая потоковая обработка для файлов больше 100 МБ
  - Обработка блоками по 64 КБ (CHUNK_SIZE)
  - Поддержка всех режимов (ECB, CBC, CFB, OFB, CTR)
  - GCM пока использует не-потоковую обработку (требуется полный доступ к данным для GHASH)
  
- **Промежуточные файлы:**
  - Создание временных файлов для отслеживания прогресса
  - Возможность продолжения после сбоя (временные файлы сохраняются)
  - Атомарная запись результата (перемещение временного файла в финальное местоположение)
  - Автоматическая очистка временных файлов при успешном завершении

- **Новые функции:**
  - `aes_encrypt_stream()` — потоковое шифрование
  - `aes_decrypt_stream()` — потоковое дешифрование
  - `create_temp_file()` — создание временных файлов
  - `move_temp_to_final()` — атомарное перемещение файлов
  - `cleanup_temp_file()` — очистка временных файлов

- **Обновления CLI:**
  - Автоматическое определение размера файла
  - Выбор оптимального метода обработки (потоковый или не-потоковый)
  - Прозрачная работа для пользователя (автоматический выбор)

### Изменено

- **Архитектура I/O:**
  - Гибридный подход: потоковая обработка для больших файлов, загрузка в память для маленьких
  - Улучшенная производительность для больших файлов
  - Снижено использование памяти при обработке больших файлов

### Обновлено

- **Документация:**
  - Обновлен `README.md` с информацией о потоковой обработке
  - Обновлен `docs/API.md` с описанием потоковых функций
  - Обновлен `docs/USERGUIDE.md` с разделом о больших файлах
  - Обновлен `Технические_детали_проекта.md` с описанием новой архитектуры I/O
  - Обновлена `Пояснительная_записка_CryptoCore.md` с информацией о потоковой обработке
